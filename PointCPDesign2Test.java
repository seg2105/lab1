...
public class PointCPDesign2Test
{
  ...
  public static void main(String[] args)
  {
    ...
    try
    {
      if(args[0].toUpperCase().charAt(0) == 'C'){
        ...
        point =  new PointCPDesign2( (Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2))),Math.toDegrees(Math.atan2(y, x)) ) ;

        } else {        
        point = new PointCPDesign2(
  ...
  private static PointCPDesign2 getInput() throws IOException
  {
    ...
    if(coordType == 'C'){
      return (new PointCPDesign2( (Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2))),Math.toDegrees(Math.atan2(b, a)) ) );
    } else {
      return (new PointCPDesign2(a,b));
    }
